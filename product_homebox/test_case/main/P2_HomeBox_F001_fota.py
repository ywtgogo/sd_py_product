...
def CountDirSize():
    count = 0
    path = "%s/fota_test/0.5.%d.%d/fota_node_*" %(argv[1], cfg.upenv.ver_num, cfg.upenv.type_num)
    ...
    try:
        while 1:
           ...
           #print "honsole.console_spawn.readline(): %s\n" % honsole.console_spawn.readline()
            ...
            timeout2 = timeout1 + datetime.timedelta(minutes=5)
            while down_fin:
        ...
        print "Succ: %d,  Fail: %d, Total: %d" % (succ_cnt, fail_cnt, (cir_cnt))
        honsole.log("Succ: %d,  Fail: %d, Total: %d" % (succ_cnt, fail_cnt, (cir_cnt)))
